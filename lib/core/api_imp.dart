...
class ApiImpl
{
  ...
  Future<ApiResponse> login(
    ...
    ApiResponse response = await _client.post(AppConstants.NGROK_LOGIN,
  ...
  Future<ApiResponse> get_attendance() async {
    ...
    ApiResponse response = await _client.get(await AppConstants().getAccordingGettterUrl(),
        headers: <String, String>{},).onError((error, stackTrace) {
...
class MyClient {
  ...
  Future<ApiResponse> _generateResponse(Response res) async {
    ...
    try {
      ...
      _apiResponse.status = res.statusCode;
      _apiResponse.msg = data["errors"] ?? "no msg";
      _apiResponse.success = _apiResponse.status == 200 ? true : false;
      _apiResponse.data =
      _apiResponse.status != 200 ? null : json.decode(res.body);
    } on SocketException {
